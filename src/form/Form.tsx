...
export type ObjectQuestionChildren<T, Q extends ObjectQuestion> = {
  [key in keyof Q["props"]["fields"]]: T;
};
...
type FormRendererProvider<T> = (
  spec: FormRendererSpec<T>
) => FormRenderer<T, Question>;

const provideFormRenderer: FormRendererProvider<ReactNode> =
  (spec) => (tags, question, answer, onChange) => {
    if (question) {
      switch (question.type) {
        case "Object": {
          const children = Object.entries(question.props.fields).reduce(
            (res, curr) => {
              ...
              return {
                ...
                [name]: provideFormRenderer(spec)(
                  ...
                  child,
                  answer
                    ? (answer as ObjectAnswer<ObjectQuestion>)[name]
                    : undefined,
                  (newAnswer) =>
                    onChange?.(
                      answer
                        ? { ...(answer as object), [name]: newAnswer }
                        : { [name]: newAnswer }
                    )
                ),
              };
          ...
          return spec.renderObject(
            children,
            tags,
            question.props,
            answer as ObjectAnswer<ObjectQuestion>
          );
        }
        case "Text":
          return spec.renderText(
            tags,
            question["props"],
            answer as TextAnswer,
            onChange
          );
        case "Checkboxes":
        ...
        case "YesNo":
          const children = {
            child:
              ...
              (answer === true ||
                (answer as YesNoAnswerWithChildren)?.value === true)
                ? provideFormRenderer(spec)(
                    tags,
                    question["props"].ifYes,
                    answer
                      ? (answer as YesNoAnswerWithChildren).ifYes
                      : undefined,
                    (newAnswer) => {
                      onChange?.(
                        answer
                          ? { ...(answer as object), ifYes: newAnswer }
                          : { ifYes: newAnswer }
                      );
                    }
                  )
                : question["props"].ifNo &&
                  (answer === false ||
                    (answer as YesNoAnswerWithChildren)?.value === false)
                ? provideFormRenderer(spec)(
                    tags,
                    question["props"].ifNo,
                    answer
                      ? (answer as YesNoAnswerWithChildren).ifNo
                      : undefined,
                    (newAnswer) =>
                      onChange?.(
                        answer
          ...
          const onYesNoChange =
            question["props"].ifYes || question["props"].ifNo
              ? (newAnswer: YesNoAnswer) =>
                  onChange?.(
                    answer
                      ? { ...(answer as object), value: newAnswer }
                      : { value: newAnswer }
                  )
              : onChange;
          return spec.renderYesNo(
            children,
            tags,
            question["props"],
            answer as YesNoAnswer,
            onYesNoChange
          );
        case "Date":
          return spec.renderDate(
            tags,
            question["props"],
            answer as DateAnswer,
            onChange
          );
      }
    }
  ...
  renderDate(
    ...
    return (
      <DateInput
        {...question}
        answer={answer}
        onChange={onChange as (newAnswer: DateAnswer) => void}
      />
    );
  },
  renderText(
    ...
    return (
      <TextInput
        {...question}
        answer={answer}
        onChange={onChange as (newAnswer: TextAnswer) => void}
      />
    );
  },
  renderCheckboxes(
    ...
    return (
      <CheckboxesInput
        {...question}
        answer={answer}
        onChange={onChange as (newAnswer: CheckboxesAnswer) => void}
      />
    );
  },
  renderSelect(
    ...
    return (
      <SelectInput
        {...question}
        answer={answer}
        onChange={onChange as (newAnswer: SelectAnswer) => void}
      />
    );
  },
  renderYesNo(
    ...
    return (
      <YesNoInput
        ...
        {...question}
        answer={answer}
        onChange={onChange as (newAnswer: YesNoAnswer) => void}
      />
    );
  },
  renderObject(
          ...
          .map(([name, child], index) => (
            <div key={index}>
              {cloneElement(child as any, {
                name,
                answer: answer ? answer[name] : undefined,
              })}
            </div>
          ))}
      </div>
...
export const isCompleted = provideFormStatusRenderer({
  ...
  renderObject(
    ...
    if (question?.optional) {
      ...
      return (
        Object.keys(question.fields).every((e) => answerKeys.includes(e)) &&
        Object.values(children).every((e) => e)
      );
    }
  },
});
